-- Clearwave 接口任务定义、分配和 occurrence 事实表。
-- occurrence 使用 task/member/scheduled_date 复合自然键，禁止对外暴露 surrogate ID。

CREATE TABLE IF NOT EXISTS kids_task_definition (
    ...
    task_tag_id VARCHAR(128) NULL COMMENT '接口任务标签标识',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='kids接口任务定义';

CREATE TABLE IF NOT EXISTS kids_task_assignment (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '主键',
    task_id VARCHAR(128) NOT NULL COMMENT '接口任务标识',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='kids接口任务成员分配';

CREATE TABLE IF NOT EXISTS kids_task_occurrence (
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='kids接口任务 occurrence';
