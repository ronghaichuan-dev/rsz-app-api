...
// ValidateV1Request 按嵌入的 OpenAPI 合同校验当前 operation 的 header、path、query 和 body。
...
// loadV1Spec 只解析一次内嵌合同，避免请求时读取外部项目文件。
...
// findV1Operation 根据冻结 operation ID 和 HTTP method 找到合同 operation。
...
// findV1OperationByID 通过唯一 operation ID 查找合同 operation。
...
// validateV1Schema 递归执行合同中实际使用的 JSON Schema 2020-12 约束。
...
// v1ValidationError 返回可安全回显字段位置的稳定合同校验错误。
