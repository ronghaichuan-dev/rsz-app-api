...
const (
	// V1Version 是 Clearwave 合同主版本。
	V1Version = "1"
	// V1RequestIDHeader 是合同请求 ID 请求头。
	V1RequestIDHeader = "X-Request-Id"
	// V1VersionHeader 是合同版本请求头。
	V1VersionHeader = "X-Contract-Version"
	// V1ClientVersionHeader 是客户端版本请求头。
	V1ClientVersionHeader = "X-Client-Version"
	// V1IdempotencyHeader 是合同幂等键请求头。
	V1IdempotencyHeader = "Idempotency-Key"
)

// V1OperationInput 是 Controller 按 OpenAPI 严格解析后传给单个合同 Service 方法的输入。
...
// V1OperationOutput 是合同业务返回的数据和 HTTP 状态。
...
// V1Response 是合同成功 envelope，由统一响应 middleware 直接输出。
