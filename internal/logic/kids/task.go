...
// v1TaskTagRecordProjection 将任务标签事实还原为接口实体。
...
// v1TaskProjectionTx 将任务定义和分配表投影为接口 TaskDefinition。
...
// nullableInt64 将数据库零值转换为接口 nullable 整数。
...
// v1TaskOccurrenceProjection 将 occurrence 数据库记录投影为接口实体。
...
// v1CompletionProjection 构造 append-only completion 的接口投影。
...
// v1LedgerProjection 构造 append-only 星星流水的接口投影。
...
// v1LedgerRecordProjection 将持久化流水还原为接口 LedgerEntry。
