...
// GetCurrentAccount 获取当前账号合同 bootstrap。
...
// CreateInviteGuestSession 创建合同游客会话。
...
// UpdateCircleWithVersion 更新合同圈子。
...
// DeleteCircleWithVersion 删除合同圈子。
...
// UpsertCircleAdministrator 更新合同管理员。
...
// DeleteCircleAdministrator 删除合同管理员。
...
// RedeemAdministratorInvite 兑换管理员合同邀请。
...
// UpsertTaskTagWithVersion 更新合同任务标签。
...
// DeleteTaskTagWithVersion 删除合同任务标签。
...
// ListTaskOccurrences 查询合同任务 occurrence。
...
// CompleteTaskWithVersion 完成合同任务。
...
// CancelTaskCompletionWithVersion 取消合同任务完成记录。
...
// ListTaskCompletionDetails 查询合同任务完成明细。
...
// AdjustMemberStarsWithVersion 调整合同成员星星。
...
// Unavailable 返回尚未实现的合同 operation 错误。
...
// v1ReplayOutput 复制首次快照，并将存在的合同回执标记为安全幂等重放。
...
// resolveV1Principal 从持久化 session 解析合同认证上下文，拒绝过期、撤销或类型不匹配的凭据。
...
// v1GetCurrentEntitlement 只返回已由服务端验证并持久化的权益快照；缺失时明确返回不可用。
func (s *sKids) v1GetCurrentEntitlement(ctx context.Context, in v1.V1OperationInput) (map[string]any, string, error) {
	...
	if row.IsEmpty() {
		return nil, "", v1Error(503, "UNAVAILABLE", true, "verified entitlement is unavailable")
	}
	return map[string]any{"entitlement_id": row["entitlement_id"].String(), "account_id": row["account_id"].String(), "status": row["status"].String(), "trial_eligible": false, "product_id": nullableString(row["product_id"].String()), "plan_code": nil, "base_plan_id": nil, "offer_id": nil, "billing_phase": "unknown", "valid_until_ms": v1NullableTimeMillis(row["valid_until_at"].Time()), "verified_at_ms": row["verified_at"].Time().UnixMilli(), "revoked_at_ms": v1NullableTimeMillis(row["revoked_at"].Time()), "version": row["version"].Int64()}, "", nil
}

// v1GetCircleBootstrap 返回指定圈子的同一快照分页，所有实体均来自合同事实表。
...
// v1DeleteMember 将成员软删除并保留合同 tombstone；任务和奖励领域尚未接入时不会存在关联指派。
...
// v1EntityTombstone 构造合同通用实体 tombstone。
...
// v1AdministratorRecordProjection 将管理员记录还原为合同管理员实体。
...
// v1MemberRecordProjection 将成员记录还原为合同成员实体。
...
// v1ListV1Circles 从合同成员身份和圈子事实读取当前账号的分页圈子列表。
...
// v1PageCursor 为合同分页响应签发下一页的服务端不透明游标。
...
// v1CircleRecordProjection 将合同圈子数据库记录还原为冻结 wire 投影。
...
// v1MembershipRecordProjection 将合同成员身份数据库记录还原为冻结 wire 投影。
...
// v1NullableTimeMillis 将可空数据库时间转换为合同 epoch 毫秒字段。
...
// v1AdministratorProjection 构造新建所有者管理员的合同投影。
...
// v1MemberProjection 构造 onboarding 初始成员的合同投影。
...
// v1AccountRecordProjection 还原合同账号投影。
...
// v1AccountBindingRecordProjection 还原合同账号绑定投影。
...
// v1InviteRecordProjection 将邀请码事实投影为不含 code secret 的合同 outgoing invite。
...
// v1MembershipProjectionForActor 构造管理员或成员加入后的合同 membership 投影。
...
// v1ListCircles 从当前 kids 圈子投影构造合同分页数据。
...
// v1ExchangeHistory 返回合同兑换历史分页。
func (s *sKids) v1ExchangeHistory(ctx context.Context, in v1.V1OperationInput) (map[string]any, string, error) {
	return map[string]any{"items": []any{}, "next_cursor": nil, "has_more": false, "snapshot_cursor": v1Cursor(), "total_exchanges": 0}, "", nil
}

// v1Statistics 返回符合合同时间和分页语义的统计骨架。
...
// v1SubmitFeedback 持久化结构化反馈并创建合同回执。
...
// v1Mutation 统一生成持久化合同回执，领域迁移期间保持稳定 wire 结构。
...
// v1CreateGuestSession 创建合同游客会话并把 token 摘要持久化。
...
// v1ExchangeGoogleProof 在真实 Google 配置缺失时明确返回不可用。
func (s *sKids) v1ExchangeGoogleProof(ctx context.Context, in v1.V1OperationInput) (map[string]any, string, error) {
	if strings.TrimSpace(fmt.Sprint(in.Body["google_id_token"])) == "" || strings.TrimSpace(fmt.Sprint(in.Body["proof_nonce"])) == "" {
...
// v1SessionMutation 处理合同 refresh/revoke 的持久化状态。
...
// v1PrepareAsset 在对象存储未配置时明确拒绝签发不可用的上传目标。
func (s *sKids) v1PrepareAsset(ctx context.Context, in v1.V1OperationInput) (map[string]any, string, error) {
	return nil, "", v1Error(503, "UNAVAILABLE", true, "object storage is not configured")
}

// v1CommitAsset 在对象存储未配置时拒绝伪造已提交资产。
...
// v1IdempotencySave 保存合同首次响应，保证断线重放返回同一结果。
...
func v1CreateCommitTx(ctx context.Context, tx gdb.TX, circleID, operationID string, changes map[string]any) (map[string]any, error) {
	...
	receiptID := v1ID("receipt", uuid.NewString())
	changePayload, err := json.Marshal(changes)
	if err != nil {
		return nil, err
	}
...
// v1Receipt 构造符合合同字段集的稳定回执，持久化由具体事务负责。
...
// v1ID 将内部标识稳定映射为合同要求的 UUID 型 opaque ID。
...
// v1Cursor 创建满足合同 pattern 的不透明同步游标。
...
// nullableString 将空字符串映射为合同 nullable 字段。
...
// nullableV1String 将 JSON null 映射为可安全写入合同文本列的空字符串。
...
// v1Secret 生成仅用于合同 credential 的随机秘密。
