...
- 后续编码必须优先考虑 SQL 查询效率和整体服务性能；新增查询前应明确数据规模、访问频率、筛选条件、排序方式和分页边界。
- 高频或列表查询必须只读取实际需要的字段，禁止无必要的 `SELECT *`、全表扫描和在应用层加载大量数据后再过滤或排序。
- `WHERE`、`JOIN`、`ORDER BY`、范围查询和分页使用的字段必须评估并补充合适索引；复合索引的字段顺序应与实际查询条件和排序顺序一致。
- 禁止在循环中逐条查询关联数据形成 N+1 查询；应优先使用批量 `IN` 查询、关联查询或预取后在内存中按主键映射。
- 大数据量列表必须使用有边界的分页；优先使用稳定 cursor/seek 分页，避免高 offset 深分页。接口必须限制 page size，防止一次请求占用过多数据库连接、内存或网络带宽。
- 事务只包含保证一致性所需的最小读写范围；避免在事务中调用外部服务、做耗时计算或持有不必要的更新锁。多资源更新须保持固定加锁顺序，降低死锁概率。
- 新增或修改关键、高频、慢查询风险 SQL 时，必须在目标数据库使用 `EXPLAIN` 或等价执行计划检查索引命中、扫描行数和排序/临时表风险；发现全表扫描或索引失效时应先优化查询或索引再交付。
- 缓存、读模型和批量接口可用于性能优化，但不得替代当前微服务数据库中的业务事实；缓存失效、超时和降级必须有明确边界，不得返回错误的业务成功结果。
